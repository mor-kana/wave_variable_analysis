...
print(t)
# 結果を格納する配列の初期化
delta_m_value = np.zeros((num_point,2,1), dtype=float)
delta_s_value = np.zeros((num_point,2,1), dtype=float)
omega_m_value = np.zeros((num_point,2,1), dtype=float)
omega_s_value = np.zeros((num_point,2,1), dtype=float)
...
for i in range(num_point):
    ...
    if current_time >= 0:
        ...
        #print(omega_m_value[i])
        omega_m_value[i] = b * delta_m_value[i-1] - np.sqrt(2*b) * vm_value[i]

        if np.any(delta_m_value[idx_minus_2] == 1):
            delta_m_value[i] = np.zeros((2, 1))
        else:
            delta_m_value[i] = operator_input
...
#########################################################################< グラフ設定 >#####################################################################################
delta_m_v = np.zeros(num_point, dtype=float)
delta_s_v = np.zeros(num_point, dtype=float)
omega_m_p = np.zeros(num_point, dtype=float)
omega_s_p = np.zeros(num_point, dtype=float)
...
delta_m_omega = np.zeros(num_point, dtype=float)
delta_s_omega = np.zeros(num_point, dtype=float)
omega_m_theta = np.zeros(num_point, dtype=float)
omega_s_theta = np.zeros(num_point, dtype=float)
...
delta_m_v = np.array([x[0, 0] for x in delta_m_value])
delta_s_v = np.array([x[0, 0] for x in delta_s_value])
omega_m_p = np.array([x[0, 0] for x in omega_m_value])
omega_s_p = np.array([x[0, 0] for x in omega_s_value])
...
delta_m_omega = np.array([x[1, 0] for x in delta_m_value])
delta_s_omega = np.array([x[1, 0] for x in delta_s_value])
omega_m_theta = np.array([x[1, 0] for x in omega_m_value])
omega_s_theta = np.array([x[1, 0] for x in omega_s_value])
...
l1, l2, l3, l4 = r"$\delta_{mv}$ [m/s]", r"$u_{mv}$ [m/s]", r"$u_{sv}$ [m/s]", r"$\delta_{sv}$ [m/s]"
l5, l6, l7, l8 = r"$\omega_{sp}$ [m", r"$v_{sp}$ [m]", r"$v_{mp}$ [m]", r"$\omega_{mp}$ [m]"
l9, l10, l11, l12 = r"$\delta_{m\omega}$ [rad/s]", r"$u_{m\omega}$ [rad/s]", r"$u_{s\omega}$ [rad/s]", r"$\delta_{s\omega}$ [rad/s]"
l13, l14, l15, l16 = r"$\omega_{s\theta}$ [rad]", r"$v_{s\theta}$ [rad]", r"$v_{m\theta}$ [rad]", r"$\omega_{m\theta}$ [rad]"

# 関数のプロット
ax1.plot(t, delta_m_v, color=c1, label=l1, marker='.')
ax2.plot(t, omega_m_p, color=c8, label=l8, marker='.')
...
ax13.plot(t, delta_s_v, color=c4, label=l4, marker='.')
ax14.plot(t, omega_s_p, color=c5, label=l5, marker='.')

ax3.plot(t, delta_m_omega, color=c1, label=l9, marker='.')
ax4.plot(t, omega_m_theta, color=c8, label=l16, marker='.')
...
ax15.plot(t, delta_s_omega, color=c4, label=l12, marker='.')
ax16.plot(t, omega_s_theta, color=c5, label=l13, marker='.')
...
for i, ax in enumerate(ax_list):
   # ax.legend(loc='upper right')
    ax.set_ylim(-3, 3)
    ax.set_ylabel(label_list[i],fontproperties=font,fontsize=10)
    ax.set_xlabel('Time [s]', fontproperties=font,fontsize=10)
    setGridPrefered(ax)
